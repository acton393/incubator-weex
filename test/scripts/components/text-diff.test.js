...
describe('border test', function () {
  ...
  before(function () {
    return util.init(driver)
      .get(util.getPage('/components/text.js'))
  ...
  it('#1 border screenshot diff', () => {
    ...
    .then(imgData => {
      ...
      var oldImgPath = path.join(screenshotFolder, process.env.platform === 'android' ? 'text-android.png' : 'text-ios.png');
      var diffImgPath = path.join(screenshotFolder, process.env.platform === 'android' ? 'text-android-diff.png' : 'text-ios-diff.png');
